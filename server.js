...
<body>
    <div class="container">
        <div class="header">
            <h1>🎉 Your Landing Page is Ready!</h1>
        ...
        <div class="content">
            ...
            <ul>
                ...
                <li>✅ Custom brand colors and messaging</li>
...
function cleanHtmlResponse(htmlContent) {
  ...
  console.log('✅ HTML response cleaned');
...
async function generateWithClaude(formData) {
  ...
  console.log('🚀 Starting NEW APPROACH: Context Processing + Pure Generation');
...
Reply with: "Ready to generate ${businessName} landing page."`;
  ...
  console.log('🔍 CALL 1: Processing business context and requirements...');
  ...
  async function callClaudeAPIWithMaxTokens(prompt, maxTokens) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`🔍 Making Claude API call (attempt ${attempt}/${maxRetries})`);
        ...
        if (response.usage) {
          ...
          console.log('🔍 Token utilization:', Math.round(((response.usage.input_tokens + response.usage.output_tokens) / maxTokens) * 100) + '%');
        }
        ...
        console.log(`🔍 Error type: ${error.error?.error?.type}`);
        ...
        if (error.status === 529 && attempt < maxRetries) {
          ...
          console.log(`⏳ Server overloaded. Waiting ${waitTime}ms before retry ${attempt + 1}/${maxRetries}...`);
  ...
  async function callClaudeAPIWithHistory(messages) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`🔍 Making Claude API call with history (attempt ${attempt}/${maxRetries})`);
        ...
        if (response.usage) {
          ...
          console.log('🔍 Output tokens:', response.usage.output_tokens);
          console.log('🔍 Total tokens:', response.usage.input_tokens + response.usage.output_tokens);
          console.log('🔍 Max tokens allowed: 8192');
          console.log('🔍 Token utilization:', Math.round(((response.usage.input_tokens + response.usage.output_tokens) / 8192) * 100) + '%');
          ...
          console.log('🔍 Character to token ratio:', Math.round(totalChars / totalTokens * 100) / 100, 'chars per token');
  ...
  async function callClaudeAPI(prompt, expectJson = false) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`🔍 Making Claude API call (attempt ${attempt}/${maxRetries}) with model: claude-sonnet-4-20250514`);
        ...
        if (response.usage) {
          ...
          console.log('🔍 Output tokens:', response.usage.output_tokens);
          console.log('🔍 Total tokens:', response.usage.input_tokens + response.usage.output_tokens);
          console.log('🔍 Max tokens allowed:', expectJson ? 2000 : 8192);
          console.log('🔍 Token utilization:', Math.round(((response.usage.input_tokens + response.usage.output_tokens) / (expectJson ? 2000 : 8192)) * 100) + '%');
  ...
  try {
    ...
    console.log('🔍 Context response length:', contextResponse.length, 'characters');
    console.log('🔍 Context response preview:', contextResponse.substring(0, 200) + '...');
...
Return only HTML code starting with <!DOCTYPE html>:`;
    ...
    console.log('🔍 CALL 2: Generating complete landing page...');
    console.log('🔍 Generation prompt length:', generationPrompt.length);
    ...
    console.log('✅ Pure generation completed');
    console.log('🔍 Final output length:', finalHtml.length, 'characters');
    ...
    if (!finalHtml.includes('<!DOCTYPE html>') || !finalHtml.includes('</html>')) {
      console.log('⚠️ Generation failed - invalid final HTML');
      console.log('🔍 Response preview:', finalHtml.substring(0, 500));
    ...
    console.log('📊 NEW APPROACH GENERATION SUMMARY:');
    console.log('🔍 Context processing length:', contextResponse.length, 'characters');
    console.log('🔍 Final generation length:', finalHtml.length, 'characters');
    console.log('🔍 Business name occurrences:', businessNameCount);
    console.log('🔍 "Your Business" occurrences:', yourBusinessCount);
    console.log('🔍 CSS styling present:', hasCSS ? 'YES' : 'NO');
    console.log('🔍 JavaScript present:', hasJS ? 'YES' : 'NO');
    console.log('🔍 Primary color used:', primaryColor);
    console.log('🔍 Secondary color used:', secondaryColor);
    
    if (businessNameCount < 5) {
      console.log('⚠️ Business name integration may be low');
    }
    if (!hasCSS) {
      console.log('⚠️ CSS styling may be missing');
    }
    ...
    console.error('❌ NEW APPROACH Error after retries:', error.message);
    console.error('🔍 Error status:', error.status);
    console.error('🔍 Error type:', error.error?.error?.type);
...
app.get('/health', (req, res) => {
  ...
  console.log('🏥 Health check requested:', healthCheck.timestamp);
...
app.post('/api/claude', async (req, res) => {
  try {
    ...
    console.log('🔍 Direct Claude API call received');
...
app.post('/api/generate-elite-landing', async (req, res) => {
  console.log('🚀 Elite Landing Page Generation Request');
...
const server = app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 CliqPages server running on port ${PORT}`);
